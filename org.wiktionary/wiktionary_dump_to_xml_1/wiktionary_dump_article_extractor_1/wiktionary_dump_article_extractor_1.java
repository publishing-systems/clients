...
public class wiktionary_dump_article_extractor_1
{
    ...
    public int extract(String args[])
    {
        ...
        String programPath = wiktionary_dump_article_extractor_1.class.getProtectionDomain().getCodeSource().getLocation().getPath();
        ...
        File replacementDictionaryFile = new File(programPath + File.separator + "txtreplace1_replacement_dictionary.xml");

        if (replacementDictionaryFile.exists() != true)
        {
            throw constructTermination("messageTxtreplace1ReplacementDictionaryFileDoesntExist", null, null, replacementDictionaryFile.getAbsolutePath());
        }

        if (replacementDictionaryFile.isFile() != true)
        {
            throw constructTermination("messageTxtreplace1ReplacementDictionaryPathIsntAFile", null, null, replacementDictionaryFile.getAbsolutePath());
        }

        if (replacementDictionaryFile.canRead() != true)
        {
            throw constructTermination("messageTxtreplace1ReplacementDictionaryFileIsntReadable", null, null, replacementDictionaryFile.getAbsolutePath());
        }
                            ...
                            if (sbNs.toString().equals("0") == true)
                            {
                                if (sbId != null)
                                {
                                    if (sbText != null)
                                    {
                                        ...
                                        /**
                                         * @todo Replacement could be done on the entire input file to avoid the invocations per article,
                                         *     but that shouldn't be done on the original file which is supposed to stay unchanged, and
                                         *     a temporary copy of the dump file would cost hard drive capacity.
                                         */
/*
                                        ProcessBuilder builder = new ProcessBuilder("java", "txtreplace1", outFile.getAbsolutePath(), replacementDictionaryFile.getAbsolutePath(), outFile.getAbsolutePath());
                                        ...
                                        {
                                            throw constructTermination("messageTxtreplace1ErrorWhileReadingOutput", ex, null);
                                        }
*/
